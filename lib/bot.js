...
const searchRegex = /\/search@freefilessbot (piratebay|limetorrent|1337x) (.+)/;
const detailsRegex = /\/details@freefilessbot (piratebay|limetorrent|1337x) (.+)/;
const downloadRegex = /\/download@freefilessbot (.+)/;
const statusRegex = /\/status@freefilessbot (.+)/;
const removeRegex = /\/remove@freefilessbot (.+)/;
...
function bot(torrent, bot) {
  bot.onText(/\/start@freefilessbot/, async msg => {
  ...
  bot.on("message", async msg => {
    ...
    try {
      ...
      bot.sendMessage(-1001236396940, `${fileName} upload successful\nDrive link: ${driveLink}\nPublic link: ${publicLink}`);
    } catch (e) {
      bot.sendMessage(-1001236396940, e.message || "An error occured");
    }
  });

  bot.onText(/\/server diskinfo@freefilessbot (.+)/, async (msg, match) => {
    const from = -1001236396940
    const path = match[1];
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const onDriveUpload = (torr, url) => bot.sendMessage(-1001236396940, `${torr.name} uploaded to drive\n${url}`);
    const onDriveUploadStart = torr => bot.sendMessage(-1001236396940, `Uploading ${torr.name} to gdrive`);
